...
export const inventoryService = {
  getAllItems: async (params = {}) => {
    // Default to large pageSize to fetch all items for client-side processing
  ...
  getAvailableItems: async (params = {}) => {
    // Default to large pageSize to fetch all available items
    if (!params.pageSize) params.pageSize = 10000;
    const query = new URLSearchParams(params).toString();
  ...
  getLentOutItems: async (params = {}) => {
    // Default to large pageSize to fetch all lent-out items
    if (!params.pageSize) params.pageSize = 10000;
    const query = new URLSearchParams(params).toString();
  ...
  getItemsByBorrower: async (borrowerID) => {
    const data = await apiRequest(`/items?currentBorrower=${borrowerID}&status=In-use&pageSize=10000`);
    return (data.items || []).map(item => ({ ...item, id: item.itemId }));
  },

  getItemsByVendor: async (vendor) => {
    const data = await apiRequest(`/items?supplier=${encodeURIComponent(vendor)}&pageSize=10000`);
    return (data.items || []).map(item => ({ ...item, id: item.itemId }));
  },

  getComponentsForMother: async (motherID) => {
    // Use the dedicated components endpoint instead of fetching all items
    try {
      const data = await apiRequest(`/items/${motherID}/components`);
      return (data.components || []).map(item => ({ ...item, id: item.itemId }));
    } catch {
      return [];
    }
  },

  // Dedicated export endpoint for large dataset Excel export
  exportAllItems: async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    const data = await apiRequest(`/items/export?${query}`);
    return (data.items || []).map(item => ({ ...item, id: item.itemId }));
  },
  ...
  searchItems: async (query) => {
    const data = await apiRequest(`/items?search=${encodeURIComponent(query)}&pageSize=10000`);
    return (data.items || []).map(item => ({ ...item, id: item.itemId }));
  },
...
export const borrowingService = {
  ...
  getRequestsForUser: async (borrowerID) => {
    const data = await apiRequest(`/borrow-requests/my?pageSize=10000`);
    return data.requests || [];
  },
  ...
  getTopLevelPendingCount: async () => {
    // Use lightweight count endpoint instead of fetching all pending requests
    const data = await apiRequest('/borrow-requests/pending-count');
    return data.count || 0;
  },
