...
import { inventoryService, borrowingService, userService } from '../utils/services'
...
export default {
  ...
  setup() {
    ...
    // Dynamic user name cache - built from API data
    const userNameCache = ref({})

    const getBorrowerName = (id) => {
      if (!id) return '-'
      if (userNameCache.value[id]) return userNameCache.value[id]
      // Fetch async and cache for next render
      userService.getUserById(id).then(u => {
        if (u) userNameCache.value[id] = u.name
      }).catch(() => {})
      return id
    }
    ...
    const loadLentOutItems = async () => {
      try {
        ...
        const [lentOut, reqs] = await Promise.all([
        ...
        // Pre-populate user name cache from request data
        reqs.forEach(r => {
          if (r.borrowerID && r.borrowerName) {
            userNameCache.value[r.borrowerID] = r.borrowerName
          }
        })
      } catch (e) {
