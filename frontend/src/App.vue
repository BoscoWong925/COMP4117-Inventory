  ...
  <div class="container-main" v-if="isAuthenticated">
    ...
    <footer class="border-t border-gray-200 bg-white py-4 mt-8">
      <div class="max-w-7xl mx-auto px-4 text-center text-gray-600 text-sm">
        <p>University COMP Department Inventory System © 2026</p>
      </div>
    </footer>
  </div>

  <LoginPage v-else :onLogin="handleLogin" />
</template>
...
export default {
  ...
  setup() {
    ...
    const refreshPendingCount = async () => {
      try {
        // Use lightweight count endpoint instead of fetching all requests
    ...
    const handleLogin = async (username, password) => {
      ...
      if (ok) {
        refreshPendingCount()
        if (!pollTimer) pollTimer = setInterval(refreshPendingCount, 30000)
      }
      return ok
    }
    ...
    onMounted(async () => {
      ...
      if (isAuthenticated.value) {
        refreshPendingCount()
        pollTimer = setInterval(refreshPendingCount, 30000)
      }
    })
